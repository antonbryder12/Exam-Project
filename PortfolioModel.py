...
class PortfolioModelClass:
    ...
    def simulate(self,R=None):
        ...
        sim.traded = traded
    def summary(self):
        """ the six numbers to report for the current simulation """
        par = self.par
        sim = self.sim
        n_trades = (sim.traded > 0).sum(axis=1).mean()
        avg_distance = np.abs(sim.theta[:,:-1] - par.theta_star).mean()
        WT = sim.W[:,-1]
        mean_WT = WT.mean()
