...
class SolowModelClass:
    ...
    def evaluate(self,s0,phi):
        ...
        sim = self.simulate(s)
        W = self.welfare(sim.c)

        return W

    # the savings path with a free long-run level: s_t = s_inf + (s0-s_inf)*phi**t
    def s_path3(self,s0,phi,s_inf):
    ...
    # welfare of the three-parameter savings rule (s0,phi,s_inf)
    def evaluate3(self,s0,phi,s_inf):

        s = self.s_path3(s0,phi,s_inf)
        sim = self.simulate(s)
        W = self.welfare(sim.c)

        return W

    # alternative savings path using exponential decay: s_t = s_bar + (s0-s_bar)*exp(-lam*t)
    def s_path_alt(self,s0,lam):

        par = self.par

        t = np.arange(par.T)
        s = par.s_bar + (s0-par.s_bar)*np.exp(-lam*t)

        return s

    # welfare of the alternative savings rule (s0,lam)
    def evaluate_alt(self,s0,lam):

        s = self.s_path_alt(s0,lam)
        sim = self.simulate(s)
        W = self.welfare(sim.c)
